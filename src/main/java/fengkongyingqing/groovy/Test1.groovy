...
/**
 * @FileName: Test1.java* @Description: Test1.java类说明* @Author: wei.tang* @Date: 2020/6/11 15:35
 */
class Test1 {
    static void main(String[] args) {

        println main1(source())
    }

    static int main1(String sourceStr) {
        int count = 0

        try {
            String sources = JSON.toJSON(sourceStr)
            println sources
            char[] tempArr = sources.toCharArray()
            ...
            Map<String, List<Map<String, String>>> source = JSON.parse(sourceStr)
            List<Map<String, String>> result = source["ICR_POQ_OTHERQUERY"]
            result.each {
